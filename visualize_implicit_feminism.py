...
# Title
ax.text(5, 9.5, 'Strategic Terminology: "Postmodern Neo-Marxists" as Implicit Reference', 
        ha='center', fontsize=18, fontweight='bold')
ax.text(5, 9.1, 'YouTube Video Avoids "Feminism" While Describing Radical Feminist Ideology', 
        ha='center', fontsize=12, style='italic', color='#555')

# LEFT SIDE: YouTube Video
# Box for YouTube
youtube_box = FancyBboxPatch((0.3, 6.5), 4, 2, 
                             boxstyle="round,pad=0.1", 
                             edgecolor='#FF0000', facecolor='#FFE6E6', 
                             linewidth=3)
ax.add_patch(youtube_box)

ax.text(2.3, 8.2, 'YOUTUBE VIDEO', ha='center', fontsize=14, fontweight='bold', color='#CC0000')
ax.text(2.3, 7.9, 'Jordan Peterson - Postmodernism', ha='center', fontsize=10, style='italic')
ax.text(2.3, 7.5, 'Uses term:', ha='center', fontsize=11, fontweight='bold')
ax.text(2.3, 7.15, '"Postmodern Neo-Marxists"', ha='center', fontsize=12, 
        bbox=dict(boxstyle='round', facecolor='yellow', alpha=0.3))
ax.text(2.3, 6.8, 'Mentions "feminism": 0 times', ha='center', fontsize=9, 
...
for char in characteristics_yt:
    ax.text(2.3, y_pos, char, ha='center', fontsize=9, color='#000')
    y_pos -= 0.35

# RIGHT SIDE: 2083 Document
# Box for 2083
doc_box = FancyBboxPatch((5.7, 6.5), 4, 2, 
                         boxstyle="round,pad=0.1", 
                         edgecolor='#0066CC', facecolor='#E6F2FF', 
                         linewidth=3)
ax.add_patch(doc_box)

ax.text(7.7, 8.2, '2083 DOCUMENT', ha='center', fontsize=14, fontweight='bold', color='#0066CC')
ax.text(7.7, 7.9, 'European Declaration', ha='center', fontsize=10, style='italic')
ax.text(7.7, 7.5, 'Uses term:', ha='center', fontsize=11, fontweight='bold')
ax.text(7.7, 7.15, '"Radical Feminism"', ha='center', fontsize=12, 
        bbox=dict(boxstyle='round', facecolor='lightblue', alpha=0.3))
ax.text(7.7, 6.8, 'Mentions "feminism": 75 times', ha='center', fontsize=9, 
        color='#0066CC', fontweight='bold')

# Characteristics of "radical feminism" in 2083
ax.text(7.7, char_y, 'Characteristics Described:', ha='center', fontsize=11, 
        fontweight='bold', color='#333')
characteristics_doc = [
...
for char in characteristics_doc:
    ax.text(7.7, y_pos, char, ha='center', fontsize=9, color='#000')
    y_pos -= 0.35

# CENTER: Overlap analysis
overlap_box = FancyBboxPatch((2, 2.5), 6, 1.5, 
                             boxstyle="round,pad=0.1", 
                             edgecolor='#00AA00', facecolor='#E6FFE6', 
                             linewidth=3)
ax.add_patch(overlap_box)

ax.text(5, 3.7, 'CONCEPTUAL OVERLAP: 6 Shared Characteristics', 
...
for i, item in enumerate(shared):
    if i < 3:
        ax.text(col1_x, y_start - (i * 0.3), item, ha='center', fontsize=9, color='#000')
    else:
        ax.text(col2_x, y_start - ((i-3) * 0.3), item, ha='center', fontsize=9, color='#000')

# Arrows showing connection
arrow1 = FancyArrowPatch((2.3, 4.0), (5, 4.0),
                        arrowstyle='->', mutation_scale=20, 
                        color='#FF6B6B', linewidth=2)
ax.add_patch(arrow1)

arrow2 = FancyArrowPatch((7.7, 4.0), (5, 4.0),
                        arrowstyle='->', mutation_scale=20, 
                        color='#4DA6FF', linewidth=2)
ax.add_patch(arrow2)

# BOTTOM: Conclusion box
conclusion_box = FancyBboxPatch((1, 0.3), 8, 1.5, 
                                boxstyle="round,pad=0.1", 
                                edgecolor='#FF6600', facecolor='#FFF4E6', 
                                linewidth=3)
ax.add_patch(conclusion_box)

ax.text(5, 1.5, 'STRATEGIC FRAMING ANALYSIS', ha='center', fontsize=13, 
...
# Add data note
ax.text(5, 0.1, 'Data: YouTube transcript (726 words, 0 feminism mentions) vs 2083 document (41,000 words, 75 feminism mentions)', 
